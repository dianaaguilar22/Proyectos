...
function VistaCard(navigation,item) {
  return(<View style={styles.cards}>
    ...
    <Text style={styles.titulo}>{item.Observaciones} </Text>
    ...
    <Button
        ...
        <Button
          ...
          onPress={() => navigation.navigate('Ent_Ver', {item: item})}
        />
    </View>
    
  </View>)}

export default function Ent_Consultar({ navigation }) {
  const [data, setData] = useState([]);
  
 
  const getProyectos = async () =>  {
    const response = await fetch("http://192.168.100.5:5000/proyecto/entregables",{
  method: 'GET',
  headers: {
    Authorization: "Basic " + base64.encode("juan@correo.com" + ":" + "1234"),
      ...
      .then((responseJson) => {
        ...
        setData(responseJson.Entregables)
        console.log(responseJson.Entregables)
      })
      .catch((error) => {
        //Error
