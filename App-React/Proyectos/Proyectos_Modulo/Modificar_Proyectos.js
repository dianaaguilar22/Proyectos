  ...
  lista.forEach((element) => {
    elementos.push(
      <View key={contar} style={styles.datos}>
        <Text style={styles.temaText}>IdPersona: {element.Nombre} </Text>
...
export default function Modificar_Proyectos({ navigation ,route}) {
  const data = route.params.item;
  let fec = data.Fecha_Inicio.split("/")
  
  let fin = data.Fecha_Termina.split("/")
  ...
  const [temas, setTemas] = useState(data.Temas);
  const [participantes, setPar] = useState(data.Participantes);
